...
**位置**: `app.js:5365-5635`, `index.html:1276,1449-1479`, `styles.css:4507-4669`

**功能**:
- `compressShareData(data)`: Base64 壓縮分享資料（支援中文）
- `decompressShareData(encoded)`: 解壓縮分享資料
- `generateShareData()`: 收集當前歌詞和設定
- `generateShareUrl()`: 生成分享 URL
- `updateShareUrlPreview()`: 更新分享連結預覽和大小
- `showShareModal()` / `closeShareModal()`: Modal 控制
- `copyShareUrl()`: 複製分享 URL 到剪貼簿
- `loadSharedLyrics()`: 從 URL 參數載入分享的歌詞
- `initShareLyrics()`: 初始化分享系統
...
**UI 元件**:
- 🔗「分享」按鈕（青色漸變）
- 分享 Modal（URL 預覽、選項、大小顯示）
- 可選：包含 Style Prompt / 風格設定
